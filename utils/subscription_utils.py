...
def get_user_status(user_id: str, active_community_id: str = None) -> dict:
    ...
    if is_whop_user:
        ...
        if active_community_id:
            ...
            if community_res.data and str(community_res.data['owner_user_id']) == str(user_id):
                ...
                trial_queries_used = community_status.get('usage', {}).get('trial_queries_used', 0)
                trial_limit = community_status.get('limits', {}).get('owner_trial_limit', 10)

                if trial_queries_used < trial_limit:
    ...
    if active_community_id:
        ...
        if community_res.data and str(community_res.data['owner_user_id']) == str(user_id):
            ...
            if whop_company:
                role = whop_api.get_user_role_in_company(user_id, whop_company)
